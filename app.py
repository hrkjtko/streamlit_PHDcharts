...
if submit_button:
  if not filter_pass0 and not filter_pass1 and not filter_pass2 and not filter_pass3:
    ...
    filtered_table_members = filtered_df_co[(filtered_df_co['治療期間'] >= min_value) & (filtered_df_co['治療期間'] <= max_value)]['ダミーID'].unique()
    filtered_df_co = filtered_df_co[filtered_df_co['治療期間'] <= max_value]
    filtered_df_co = filtered_df_co[filtered_df_co['ダミーID'].isin(filtered_table_members)]
